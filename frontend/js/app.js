...
TarotApp.wechatNotify = function() {
  ...
  XianbaoAuth.checkAuth().then(function() {
    ...
    if (!/MicroMessenger/i.test(navigator.userAgent)) {
      self.showToast('请在微信中打开使用此功能');
      return;
    }
    self.showWechatSubscribe();
  });
};

TarotApp.showWechatSubscribe = function() {
  var self = this;
  var url = location.href.split('#')[0];
  fetch('https://auth.xianbao.online/api/auth/wechat/jssdk-config?url=' + encodeURIComponent(url), { credentials: 'include' })
    .then(function(r) { return r.json(); })
    .then(function(j) {
      if (!j.success) { self.showToast(j.error || '签名失败'); return; }
      var d = j.data;
      wx.config({
        debug: false,
        appId: d.appId,
      ...
      wx.ready(function() {
        var old = document.getElementById('wxSubscribeWrap');
        if (old) old.remove();
        var wrap = document.createElement('div');
        wrap.id = 'wxSubscribeWrap';
        wrap.style.marginTop = '12px';
        wrap.innerHTML = '<wx-open-subscribe template="4cEe8rzU3mJvxoNAqzTGRQ59wK1RLDcOb5sqQlgarRM" scene="1001">' +
          '<script type="text/wxtag-template">' +
          '<style>.wx-sub-btn{display:inline-block;padding:8px 16px;border:1px solid rgba(255,255,255,.25);border-radius:8px;background:transparent;color:#e0e0e0;font-size:13px;cursor:pointer;font-family:inherit}</style>' +
          '<button class="wx-sub-btn">🔔 微信通知我</button>' +
          '</' + 'script></wx-open-subscribe>';
        document.getElementById('interpretationActions').appendChild(wrap);
        var sub = wrap.querySelector('wx-open-subscribe');
        if (sub) {
          sub.addEventListener('subscribe', function(e) {
            if (e.detail && e.detail.errMsg && e.detail.errMsg.indexOf('ok') > -1) {
              self.sendTarotSubscribe();
            } else {
              self.showToast('未获得订阅授权');
            }
          });
          sub.addEventListener('error', function() {
            self.showToast('订阅失败，请重试');
          });
        }
      });
      wx.error(function(err) {
        self.showToast('微信配置失败: ' + (err && err.errMsg ? err.errMsg : ''));
      });
    })
    .catch(function() { self.showToast('签名请求失败'); });
};

TarotApp.sendTarotSubscribe = function() {
  var self = this;
  ...
  fetch('https://auth.xianbao.online/api/auth/wechat/subscribe/send', {
    ...
    body: JSON.stringify({
      ...
      data: { thing1: q, time4: time4, name3: '仙宝心灵成长', thing2: ans }
    })
  }).then(function(r) { return r.json(); }).then(function(j) {
    if (j.success) self.showToast('✅ 塔罗解读已发送到微信');
    else self.showToast(j.error || '发送失败');
  }).catch(function() { self.showToast('发送失败，请重试'); });
};
